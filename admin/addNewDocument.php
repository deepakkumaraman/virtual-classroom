...
if (isset($_SESSION['username']) && isset($_POST['submit'])) {
	...
	if ($validDocFormat == 1) {
       ...
       header("Location: editCourse.php");
        $_SESSION['courseEditmessage'] = "<div class='chip red black-text'>Error Adding Documents</div>";
    ...
    for ($x = 1; $x <= $numberOfDoc; $x++) 
	{
		...
		if(move_uploaded_file($_FILES["doc".$x]["tmp_name"], $filepath))
		{
            ...
            header("Location: editCourse.php");
            $_SESSION['courseEditmessage'] = "<div class='chip red black-text'>Error adding Document !!</div>";
   ...
   header("Location: editCourse.php");
    $_SESSION['courseEditmessage'] = "<div class='chip green white-text'>documents Successfully Added</div>";
    die("documents Added");
