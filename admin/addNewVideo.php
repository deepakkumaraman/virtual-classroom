...
if (isset($_SESSION['username']) && isset($_POST['submit'])) {
    ...
    */
    if ($validVidFormat == 1) {
       ...
       header("Location: editCourse.php");
        $_SESSION['courseEditmessage'] = "<div class='chip red black-text'>Error Adding Videos</div>";
    ...
    for ($x = 1; $x <= $numberOfVideos; $x++) {
        ...
        if (move_uploaded_file($_FILES["video" . $x]["tmp_name"], $filepath)) {
            ...
            header("Location: editCourse.php");
            $_SESSION['courseEditmessage'] = "<div class='chip red black-text'>Error adding video !!</div>";
   ...
   header("Location: editCourse.php");
    $_SESSION['courseEditmessage'] = "<div class='chip green white-text'>Videos Successfully Added</div>";
    die("Videos Added");
